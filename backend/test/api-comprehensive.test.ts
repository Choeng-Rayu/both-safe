...
async function request(method: string, path: string, opts: { body?: any; headers?: Record<string, string>; query?: Record<string, string>; cookie?: string } = {}) {
  const normalizedPath = path.startsWith('/') ? path.slice(1) : path;
  const base = BASE_URL.endsWith('/') ? BASE_URL : BASE_URL + '/';
  const url = new URL(normalizedPath, base);
  if (opts.query) {
    Object.entries(opts.query).forEach(([k, v]) => { if (v) url.searchParams.set(k, v); });
  }
...
async function requestRaw(base: string, method: string, path: string, opts: { body?: any; headers?: Record<string, string>; query?: Record<string, string>; cookie?: string } = {}) {
  const normalizedPath = path.startsWith('/') ? path.slice(1) : path;
...
function assertHas(obj: any, key: string, message: string) {
  assert(obj && typeof obj === 'object' && key in obj, `${message} | missing key: ${key}`);
}

function assertStatus(res: { status: number }, expected: number, message: string) {
...
async function testUserRegister() {
  const res = await request('POST', '/auth/register', {
    body: { email: 'testseller@example.com', password: 'testpassword123', name: 'Test Seller' },
  });
  // May be 409 if user already exists
  assert([201, 409].includes(res.status), `Seller register returns 201 or 409, got ${res.status}`);
  if (res.status === 201) {
...
async function testBuyerRegister() {
  const res = await request('POST', '/auth/register', {
    body: { email: 'testbuyer@example.com', password: 'testpassword123', name: 'Test Buyer' },
  });
  assert([201, 409].includes(res.status), `Buyer register returns 201 or 409, got ${res.status}`);
  if (res.status === 201) {
...
async function testBuyerLogin() {
  ...
  assert(state.buyerSessionCookie !== undefined, 'Buyer login sets session cookie');
}
...
async function testCreateDeal() {
  ...
  assertHas(res.data, 'creator_access_url', 'Create deal returns creator_access_url');
  ...
  assertEq(res.data?.status, 'AWAITING_COUNTERPARTY', 'Seller-created deal status is AWAITING_COUNTERPARTY');
  ...
  state.creatorAccessToken = new URL(creatorUrl).searchParams.get('access') || undefined;
  state.inviteToken = new URL(inviteUrl).searchParams.get('invite') || undefined;
}

async function testGetDealAsCreator() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  ...
  assertEq(res.data?.public_id, state.dealPublicId, 'Get deal public_id matches');
}
...
async function testGetDealAsInvite() {
  ...
  assertEq(res.data?.public_id, state.dealPublicId, 'Invite preview public_id matches');
}
...
async function testJoinDeal() {
  ...
  assertHas(res.data, 'participant_access_url', 'Join returns participant_access_url');
  ...
  assertEq(res.data?.status, 'AWAITING_BOTH_APPROVAL', 'Join deal status is AWAITING_BOTH_APPROVAL');
...
async function testUpdateProduct() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('PATCH', `/deals/${state.dealPublicId}/sections/product`, {
    body: {
  ...
  assertHas(res.data, 'allowed_actions', 'Update product returns allowed_actions');
  assertEq(res.data?.product?.title, 'Updated Test Product', 'Product title updated');
}

async function testUpdateParticipant() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('PATCH', `/deals/${state.dealPublicId}/sections/participant`, {
    body: {
  ...
  assertHas(res.data, 'participants', 'Update participant returns participants');
}

async function testUpdatePayout() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('PATCH', `/deals/${state.dealPublicId}/sections/payout`, {
    body: {
...
async function testApproveAsBuyer() {
  ...
  assertEq(res.data?.status, 'READY_FOR_PAYMENT', 'Status after both approve is READY_FOR_PAYMENT');
}

async function testPaymentInstruction() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('GET', `/deals/${state.dealPublicId}/payment-instruction`, {
    query: { access: state.participantAccessToken },
    cookie: state.buyerSessionCookie,
  });
  ...
  assertHas(res.data, 'expected_amount', 'Payment instruction has expected_amount');
...
async function testUploadPaymentProof() {
  ...
  const res = await request('POST', `/deals/${state.dealPublicId}/payment-proofs`, {
    body: formData,
    query: { access: state.participantAccessToken },
    cookie: state.buyerSessionCookie,
  });
  ...
  assertHas(res.data, 'message_key', 'Upload payment proof returns message_key');
  assertEq(res.data?.status, 'PAYMENT_PENDING_VERIFICATION', 'Status after payment upload');

  state.paymentId = res.data?.payment_id;
}

async function testAdminVerifyPayment() {
  if (!state.paymentId) { assert(false, 'No payment uploaded'); return; }
  const res = await request('POST', `/admin/payment-proofs/${state.paymentId}/verify`, {
    headers: { Authorization: `Bearer ${state.adminToken}` },
  });
  assertStatus(res, 201, 'Admin verify payment');
  assertHas(res.data, 'deal_status', 'Verify payment returns deal_status');
  assertEq(res.data?.deal_status, 'SELLER_PREPARING', 'Status after admin verify');
}
...
async function testUploadShippingProof() {
  ...
  const res = await request('POST', `/deals/${state.dealPublicId}/shipping-proofs`, {
    body: formData,
    query: { access: state.creatorAccessToken },
    cookie: state.sessionCookie,
  });
...
async function testConfirmReceived() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('POST', `/deals/${state.dealPublicId}/confirm-received`, {
    query: { access: state.participantAccessToken },
    cookie: state.buyerSessionCookie,
  });
  ...
  assertEq(res.data?.status, 'RELEASE_PENDING', 'Status after confirm received');
}
...
async function testOpenDispute() {
  ...
  const invite = new URL(createRes.data?.invite_url as string).searchParams.get('invite');

  // Join as buyer (with buyer session)
  const joinRes = await request('POST', `/deals/${publicId}/join`, {
    body: { invite_token: invite, role: 'buyer', name: 'Dispute Buyer', preferred_language: 'en' },
    query: { invite },
    cookie: state.buyerSessionCookie,
  });
  ...
  // Approve as seller
  const sellerAccess = new URL(createRes.data?.creator_access_url as string).searchParams.get('access');
  ...
  const paymentRes = await request('POST', `/deals/${publicId}/payment-proofs`, {
    body: paymentForm,
    query: { access: buyerToken },
    cookie: state.buyerSessionCookie,
  });
...
async function testAdminRefund() {
  ...
  const res = await request('POST', `/admin/deals/${disputedDeal.id}/refund`, {
    body: { refund_reference: 'REFUND-TEST-001', admin_note: 'Testing refund flow' },
    headers: { Authorization: `Bearer ${state.adminToken}` },
  });
...
async function testAdminAuditLog() {
  if (!state.dealPublicId) { assert(false, 'No deal created'); return; }
  const res = await request('GET', `/admin/deals/${state.dealPublicId}/audit-log`, {
    headers: { Authorization: `Bearer ${state.adminToken}` },
  });
...
async function testSwaggerDocs() {
  ...
  assert(text.includes('swagger') || text.includes('Swagger'), 'Swagger docs HTML contains swagger');
}
...
async function testCorsHeaders() {
  ...
  assert(res.status === 204 || res.headers.get('access-control-allow-origin') !== null, 'CORS headers present');
}
...
async function runTests() {
  ...
  console.log(`Total: ${state.passed + state.failed} | Passed: ${state.passed} | Failed: ${state.failed}`);
  console.log('='.repeat(60));
