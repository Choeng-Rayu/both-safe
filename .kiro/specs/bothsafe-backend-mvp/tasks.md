...
- [x] 1. Set up NestJS project infrastructure and core dependencies
...
- [x] 2. Implement Prisma database layer and schema
  - [x] 2.1 Create complete Prisma schema with all models
    ...
    - Add database indexes for performance (public_id, status, telegram_chat_id, access_token_hash)
    - Add unique constraints (one product per deal, one buyer/seller per deal)
    - _Requirements: 1, 2, 3, 15, 16, 19, 25_
  
  - [x] 2.2 Run Prisma migration and create PrismaService
    - Generate and run initial migration
    - Create PrismaService with connection lifecycle management
    - Create global PrismaModule for dependency injection
    - _Requirements: 30_

- [x] 3. Implement core utility services and constants
  - [x] 3.1 Create constants file with enums and configuration
  ...
  - [x] 3.2 Implement token generation and validation utilities
  ...
  - [x] 3.3 Implement input sanitization utilities
    - Create sanitizeText function to remove null bytes and trim whitespace
    - Create sanitizeFilename function to prevent path traversal
    - _Requirements: 21_
  
  - [x] 3.4 Create CurrentActor decorator for request context
    - Extract participant or admin from request object
    - Make available to controllers via @CurrentActor() decorator
    - _Requirements: 16_

- [x] 4. Implement global exception handling and logging
  - [x] 4.1 Create AllExceptionsFilter for consistent error responses
    - Return ErrorResponse format with statusCode, message_key, message, errors, timestamp, path, request_id
    - Handle HttpException, Prisma errors, and unknown errors
    - Log errors with sanitization (redact sensitive fields)
    - _Requirements: 21_
  
  - [x] 4.2 Create domain-specific exception classes
    - InvalidStatusTransitionException, MissingFieldsException, FieldsLockedException
    - PaymentNotReadyException, PaymentAlreadyVerifiedException
    - InvalidTokenException, InviteExpiredException
    - InvalidFileTypeException, FileTooLargeException
    - _Requirements: 6, 7, 8, 16, 18, 28_
  
  - [x] 4.3 Implement LoggingInterceptor for request/response logging
...
- [x] 5. Implement Authentication Module
  ...
  - [x] 5.5 Create AuthController with admin login endpoint
    - POST /v1/admin/login with email/password validation
    - Generate JWT access token and refresh token
    - Return tokens with expiration
    - _Requirements: 16, 22_

- [x] 6. Implement Status Engine and Missing Fields Calculator
  - [x] 6.1 Create StatusEngine class with transition validation
    ...
    - Execute side effects (ledger entries, notifications) within transaction
    - _Requirements: 6_
  
  - [x] 6.2 Create MissingFieldsCalculator class
...
- [x] 7. Implement Audit Logging Service
  - [x] 7.1 Create AuditService for immutable audit logs
  ...
  - [x] 7.2 Create AuditModule and integrate with other services
    - Export AuditService globally
    - Ensure all critical actions create audit log entries
    - _Requirements: 19_

- [x] 8. Implement Deal Module - Core Service
  - [x] 8.1 Create DealsService with deal creation
  ...
  - [x] 8.2 Implement counterparty join functionality
  ...
  - [x] 8.3 Implement deal information update methods
  ...
  - [x] 8.4 Implement deal approval functionality
  ...
  - [x] 8.5 Implement deal retrieval with calculated fields
...
- [x] 9. Implement Deal Module - API Controller
  - [x] 9.1 Create DealsController with create endpoint
  ...
  - [x] 9.5 Implement approval endpoint
    - POST /v1/deals/:publicId/approval with DealAccessGuard
    - Validate approval DTO
    - Return updated deal status
    - _Requirements: 5_

- [x] 10. Implement Ledger Module
  - [x] 10.1 Create LedgerService for append-only ledger
  ...
  - [x] 10.2 Create LedgerModule and export service
    - Make LedgerService available globally
    - _Requirements: 15_

- [x] 11. Implement Payment Module
  - [x] 11.1 Create PaymentsService for payment proof management
  ...
  - [x] 11.2 Implement admin payment verification
  ...
  - [x] 11.3 Implement admin payment rejection
  ...
  - [x] 11.4 Create PaymentsController with endpoints
    - POST /v1/deals/:publicId/payment-proofs with DealAccessGuard (buyer only)
    - Apply rate limiting (3 requests/hour per deal)
    - Handle multipart file upload
    - _Requirements: 7, 21_

- [x] 12. Implement Shipping Module
  - [x] 12.1 Create ShippingService for shipping proof management
  ...
  - [x] 12.2 Create ShippingController with upload endpoint
    - POST /v1/deals/:publicId/shipping-proofs with DealAccessGuard (seller only)
    - Handle multipart file upload for optional photos
    - _Requirements: 9_

- [x] 13. Implement Confirmation Module
  - [x] 13.1 Create ConfirmationService for buyer confirmation
  ...
  - [x] 13.2 Create ConfirmationController with endpoint
    - POST /v1/deals/:publicId/confirm-received with DealAccessGuard (buyer only)
    - _Requirements: 10_

- [x] 14. Implement Dispute Module
  - [x] 14.1 Create DisputesService for dispute management
  ...
  - [x] 14.2 Implement admin dispute resolution
...
- [x] 15. Implement Admin Module
  ...
  - [x] 15.3 Implement admin payment refund
  ...
  - [x] 15.4 Create AdminController with all admin endpoints
...
- [x] 16. Implement File Storage Module
  - [x] 16.1 Create FilesService for file upload/download
    - Implement validateFile method (type, size, extension matching)
    - Implement sanitizeFilename to prevent path traversal
    - Implement uploadFile method with object storage integration (Supabase/R2)
    - Implement generateSignedUrl for secure file access
    - Store file metadata in database
    - _Requirements: 18_
  
  - [x] 16.2 Create FilesController with upload endpoint
    - POST /v1/files/upload with authentication
    - Handle multipart file upload
    - Return file URL and metadata
    - _Requirements: 18_
  
  - [x] 16.3 Implement file access control
...
- [x] 17. Implement Notification Module
  - [x] 17.1 Create NotificationService for event-based notifications
  ...
  - [x] 17.2 Create NotificationMessageFormatter for multi-language messages
    - Define message templates for km, en, zh languages
    - Implement interpolation for dynamic data
    - _Requirements: 17, 20_
  
  - [x] 17.3 Implement BotNotifierInterface for Telegram integration
  ...
  - [x] 17.4 Create NotificationModule and export service
    - Make NotificationService available globally
    - _Requirements: 17_

- [x] 18. Implement Telegram Bot Module
  - [x] 18.1 Create BotTelegramService for Telegram Bot API integration
    - Initialize Telegram bot with token from environment
    - Implement sendMessage method
    - Implement sendNotification method with inline keyboard
    - _Requirements: 25_
  
  - [x] 18.2 Create BotStateService for conversation state management
    - Store conversation state in database
    - Implement state expiration (30 minutes)
    - Support multi-step flows (deal creation)
    - _Requirements: 25_
  
  - [x] 18.3 Create BotMessages for localized bot messages
    - Define message templates for km, en, zh
    - Support bot commands: /start, /newdeal, /mydeals, /help
    - _Requirements: 25_
  
  - [x] 18.4 Create BotUpdate handler for incoming Telegram updates
  ...
  - [x] 18.6 Create BotModule and register with NestJS
    - Configure Telegram bot module
    - Register update handlers
    - _Requirements: 25_

- [x] 19. Implement rate limiting and security middleware
  - [x] 19.1 Configure ThrottlerModule for rate limiting
  ...
  - [x] 19.2 Implement CORS configuration
    - Allow only configured frontend and admin domains
    - Enable credentials
    - Allow required headers (Content-Type, Authorization, X-Access-Token)
    - _Requirements: 21_
  
  - [x] 19.3 Implement request validation pipeline
    - Enable global ValidationPipe with class-validator
    - Whitelist unknown properties
    - Transform payloads to DTO instances
    - _Requirements: 21_

- [x] 20. Implement health check and monitoring
  - [x] 20.1 Create HealthController with health check endpoint
    - GET /health endpoint
  ...
  - [x] 20.2 Create HealthModule
    - Use @nestjs/terminus for health checks
    - _Requirements: 27_

- [x] 21. Implement deal expiration background job
  - [x] 21.1 Create ExpirationService for deal expiration
    - Implement checkExpiredDeals method
    - Find deals in DRAFT, AWAITING_COUNTERPARTY, AWAITING_BOTH_APPROVAL with expires_at < now
    - Transition to EXPIRED
    - Create audit log entries
    - _Requirements: 24_
  
  - [x] 21.2 Schedule expiration job with @nestjs/schedule
    - Run every hour
    - _Requirements: 24_
...
- [x] 23. Write integration tests for critical flows
  - [x] 23.1 Write integration tests for buyer-created deal flow
    - Test deal creation with buyer as creator
    - Test seller joining
    - Test both approvals leading to READY_FOR_PAYMENT
    - _Requirements: 1, 2, 5_
  
  - [x] 23.2 Write integration tests for payment verification flow
  ...
  - [x] 23.3 Write integration tests for dispute resolution flow
  ...
  - [x] 23.4 Write integration tests for complete happy path
...
- [x] 24. Write unit tests for core services
  - [x] 24.1 Write unit tests for StatusEngine
    - Test valid transitions allowed
    - Test invalid transitions rejected
    - Test precondition checks
    - _Requirements: 6_
  
  - [x] 24.2 Write unit tests for MissingFieldsCalculator
  ...
  - [x] 24.3 Write unit tests for TokenService
  ...
  - [x] 24.4 Write unit tests for NotificationMessageFormatter
    - Test message formatting in all languages
    - Test data interpolation
    - _Requirements: 17, 20_

- [x] 25. Write E2E tests for API endpoints
  - [x] 25.1 Write E2E test for complete buyer-created flow
...
- [x] 27. Create database seed script
  - [x] 27.1 Create seed.ts with sample data
    - Create admin user
...
- [x] 28. Create API documentation
  - [x] 28.1 Add Swagger/OpenAPI documentation
    - Install @nestjs/swagger
    - Add API decorators to all controllers
    - Generate OpenAPI spec
    - _Requirements: 26_

- [x] 29. Final integration and deployment preparation
  - [x] 29.1 Verify all environment variables documented
    - Document required variables in .env.example
    - Add validation for required variables at startup
    - _Requirements: 30_
  
  - [x] 29.2 Run full test suite and verify coverage
    - Unit: 284 passed, Integration: 7 passed, E2E: 12 passed
    - _Requirements: All_
  
  - [x] 29.3 Test with local PostgreSQL Docker container
...
- File storage integrates with Supabase Storage or Cloudflare R2
