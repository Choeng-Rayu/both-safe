...
graph TB
    ...
    Storage[Object Storage<br/>Supabase/R2]
...
3. **Data Layer**: Prisma ORM with PostgreSQL for persistence
...
const VALID_TRANSITIONS: Map<DealStatus, DealStatus[]> = {
  ...
  AWAITING_COUNTERPARTY: ['AWAITING_BOTH_APPROVAL', 'CANCELLED'],
  AWAITING_BOTH_APPROVAL: ['READY_FOR_PAYMENT', 'CANCELLED'],
  READY_FOR_PAYMENT: ['PAYMENT_PENDING_VERIFICATION', 'EXPIRED'],
...
The database uses PostgreSQL with Prisma ORM. The schema is designed for data integrity, audit trails, and efficient querying.
...
stateDiagram-v2
    ...
    READY_FOR_PAYMENT --> EXPIRED: Timeout
...
**Framework**: Jest with Test Containers (PostgreSQL)
...
- **Authentication**: Access token (header or query param)
...
**Supported Providers**:
1. **Supabase Storage** (Primary for MVP)
...
export const storageConfig = {
  ...
  supabase: {
    ...
    bucket: process.env.SUPABASE_BUCKET || 'bothsafe-files'
  },
  cloudflare: {
    accountId: process.env.CF_ACCOUNT_ID,
    accessKeyId: process.env.CF_ACCESS_KEY_ID,
    secretAccessKey: process.env.CF_SECRET_ACCESS_KEY,
    bucket: process.env.CF_BUCKET || 'bothsafe-files'
  },
  local: {
...
export class SupabaseStorageService implements StorageService {
  ...
  async generateDownloadUrl(path: string, expiresIn: number): Promise<string> {
    ...
    if (error) throw new Error(`Download URL generation failed: ${error.message}`);
...
PORT=3001
DATABASE_URL=postgresql://user:pass@localhost:5432/bothsafe_dev
...
NODE_ENV=staging
PORT=3001
DATABASE_URL=postgresql://user:pass@staging-db:5432/bothsafe_staging
...
NODE_ENV=production
PORT=3001
DATABASE_URL=postgresql://user:pass@prod-db:5432/bothsafe_prod
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_DB: bothsafe_dev
    ports:
      - "5432:5432"
    volumes:
      - postgres_data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U bothsafe"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  backend:
    ...
    ports:
      - "3001:3001"
    environment:
      DATABASE_URL: postgresql://bothsafe:dev_password@postgres:5432/bothsafe_dev
      NODE_ENV: development
    depends_on:
      postgres:
...
0 3 * * * pg_dump -U bothsafe bothsafe_prod | gzip > /backups/bothsafe_$(date +\%Y\%m\%d).sql.gz
...
- Supabase Storage has built-in versioning and backups
- Cloudflare R2 has object versioning
...
- `STORAGE_PROVIDER`: Storage provider (supabase, cloudflare, local)
