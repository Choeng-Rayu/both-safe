...
PostgreSQL runs in a **local Docker container** (already running).

- Do not create a new database service or docker-compose entry for PostgreSQL.
...
Supabase Storage or Cloudflare R2. Configure via environment variables.
For MVP, local storage fallback is acceptable during development.
...
# Backend
cd backend
cp .env.example .env   # fill DB connection to local Docker PostgreSQL
npm install
